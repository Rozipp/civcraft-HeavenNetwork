...
public class ConsumeLevelComponent extends Component {
	 ...
	 * amount entries for each item consumed for that level. For each item in the
	 * hashmap, we must have ALL of the items in the inventory. */
	...
	public void createComponent(Construct constr, boolean async) {
		...
		} else if (constr instanceof Cottage) {
			for (ConfigCottageLevel lvl : CivSettings.cottageLevels.values()) {
				this.addLevel(lvl.level, lvl.count);
				this.setConsumes(lvl.level, lvl.consumes);
			}
		} else if (constr instanceof Mine) {
	...
	private boolean hasEnoughToConsume() {
		...
		for (Integer typeID : thisLevelConsumptions.keySet()) {
			...
			if (foundCount < getConsumedAmount(requireCount)) {
				...
				/* We found an EE for this consumable, and we didn't have enough on our own. So
				 * instead, we'll try to see if we have enough if we use our EE. */
	...
	private void consumeFromInventory(Boolean sync) {
		...
		for (Integer typeID : thisLevelConsumptions.keySet()) {
			...
			if (count < amount) {
				...
				if (ee == null) {
					/* This should never happen since we should have checked for this before calling
					 * this function. */
					continue;
				} else {
					/* Lets try to use the total from the EE to supplement the base item we didn't
					 * have enough of. */
					Integer totalBase = foundCounts.get(ee.baseType);
					if (totalBase == null) {
						totalBase = 0;
					...
					int total = totalBase + (totalAlt * ee.basePerAlt);
					if (total < amount) {
						/* This should never happen since it was checked before this function in
						 * hasEnoughToConsume() */
						continue;
					}
					...
					/* Get the number of 'alt' and 'base' types we can fit into our new
					 * total/leftover. */
