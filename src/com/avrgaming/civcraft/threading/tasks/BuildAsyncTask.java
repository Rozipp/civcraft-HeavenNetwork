...
public class BuildAsyncTask extends CivAsyncTask {
	/*
	 * This task slow-builds a struct block-by-block based on the town's hammer
	 * rate. This task is per-structure building and will use the CivAsynTask
	 * interface to send synchronous requests to the main thread to build individual
	 * blocks.
	 */
	...
	public void run() {
		try {
			...
			CivLog.exception("BuildAsyncTask town:" + buildable.getTown() + " struct:" + buildable.getDisplayName()
					+ " template:" + tpl.getDirection(), e);
		}
	}
	...
	private boolean start() {
		...
		for (; buildable.getBlocksCompleted() < (tpl.size_x * tpl.size_y * tpl.size_z); buildable.blocksCompleted++) {
			...
			try {
				int nextPercentComplete = (int) (((double) buildable.getBlocksCompleted()
						/ (double) buildable.getTotalBlock()) * 100);
				if (nextPercentComplete > this.percent_complete) {
					...
					if ((this.percent_complete % 10 == 0)) {
						if (this.buildable instanceof Wonder)
							CivMessage.global(CivSettings.localize.localizedString("var_buildAsync_progressWonder",
									this.buildable.getDisplayName(), this.buildable.getTown().getName(),
									nextPercentComplete, this.buildable.getCiv().getName()));
						else
							CivMessage.sendTown(buildable.getTown(),
									CivColor.Yellow
											+ CivSettings.localize.localizedString("var_buildAsync_progressOther",
													buildable.getDisplayName(), nextPercentComplete));
					}
				}
				...
				if (buildable instanceof Wonder) {
					...
					if (buildable.isDestroyed()) {
						CivMessage.sendTown(buildable.getTown(), CivSettings.localize
								.localizedString("var_buildAsync_destroyed", buildable.getDisplayName()));
						abortWonder();
						return false;
					}
		...
		if (this.buildable instanceof Structure) {
			CivMessage.global(CivSettings.localize.localizedString("var_buildAsync_completed",
					this.buildable.getTown().getName(), "§2" + this.buildable.getDisplayName() + CivColor.RESET));
			...
			CivMessage.global(CivSettings.localize.localizedString("var_buildAsync_completedWonder",
					CivColor.Red + this.buildable.getCiv().getName() + CivColor.RESET,
					"§6" + this.buildable.getTown().getName() + CivColor.RESET,
	...
	private static int getR(int ii, int max) {
		...
		List<Integer> rm = Arrays.asList(85, 79, 44, 2, 342, 373, 496, 48, 321, 340, 330, 57, 213, 311, 201, 114, 403,
				300, 182, 431, 488, 93, 358, 501, 16, 179, 362, 348, 30, 170, 457, 183, 426, 475, 132, 291, 80, 450,
				499, 423, 464, 385, 111, 197, 410, 346, 350, 302, 375, 249, 42, 404, 411, 229, 38, 395, 202, 159, 434,
				231, 14, 441, 367, 126, 76, 95, 485, 427, 287, 169, 289, 437, 366, 120, 285, 243, 444, 506, 21, 490,
				370, 75, 344, 425, 216, 133, 361, 331, 320, 98, 4, 421, 473, 156, 290, 32, 356, 136, 34, 212, 22, 192,
				322, 260, 388, 72, 493, 415, 384, 407, 147, 489, 278, 112, 13, 89, 507, 406, 148, 308, 296, 369, 74,
				482, 50, 161, 167, 305, 119, 61, 222, 318, 293, 463, 315, 448, 430, 379, 236, 484, 41, 37, 240, 357,
				139, 31, 195, 313, 284, 155, 219, 43, 327, 137, 374, 246, 433, 476, 298, 162, 223, 368, 232, 303, 432,
				68, 199, 265, 312, 478, 466, 378, 500, 445, 247, 49, 326, 196, 19, 329, 276, 363, 189, 92, 481, 271,
				319, 63, 471, 439, 474, 332, 412, 398, 125, 171, 274, 230, 495, 66, 143, 449, 491, 163, 310, 338, 456,
				12, 337, 314, 281, 67, 418, 347, 360, 275, 154, 172, 400, 408, 181, 84, 279, 250, 46, 207, 117, 5, 286,
				90, 429, 208, 60, 349, 447, 509, 193, 18, 277, 64, 211, 301, 334, 458, 462, 96, 436, 483, 150, 188, 328,
				244, 304, 101, 194, 88, 110, 272, 380, 254, 106, 268, 498, 480, 364, 352, 414, 185, 91, 440, 242, 27,
				116, 146, 166, 469, 45, 142, 324, 173, 472, 140, 405, 461, 257, 258, 227, 335, 52, 341, 65, 339, 387,
				453, 145, 144, 217, 28, 225, 59, 391, 294, 269, 273, 77, 200, 316, 203, 51, 86, 6, 389, 135, 82, 381,
				264, 238, 24, 118, 127, 81, 168, 422, 248, 383, 309, 511, 198, 228, 397, 214, 152, 99, 58, 267, 157,
				209, 307, 508, 94, 435, 151, 345, 413, 178, 23, 54, 87, 69, 438, 252, 224, 460, 237, 420, 510, 396, 215,
				371, 175, 78, 477, 205, 29, 283, 220, 467, 109, 306, 97, 3, 492, 177, 390, 351, 376, 10, 221, 399, 503,
				251, 134, 105, 233, 377, 299, 354, 121, 486, 184, 336, 394, 39, 401, 129, 128, 35, 180, 73, 372, 113,
				138, 149, 124, 424, 504, 295, 70, 8, 115, 455, 292, 259, 55, 255, 9, 325, 459, 282, 428, 393, 270, 141,
				359, 297, 131, 280, 204, 210, 122, 36, 15, 0, 266, 235, 505, 470, 71, 186, 108, 7, 502, 443, 103, 386,
				451, 56, 165, 40, 262, 419, 317, 343, 261, 452, 454, 468, 25, 11, 20, 190, 263, 353, 107, 416, 160, 26,
				17, 241, 174, 392, 104, 164, 288, 365, 100, 218, 245, 62, 33, 187, 234, 83, 382, 497, 53, 494, 446, 409,
				355, 206, 256, 417, 158, 47, 253, 323, 1, 333, 102, 239, 191, 465, 130, 442, 226, 479, 123, 176, 153,
				487, 402);
		int ret = rm.get(i);
		while (ret >= max) {
			ret = rm.get(ret);
	...
	public boolean build() {
		...
		SimpleBlock sb;
		if (tpl.size_x > 512 || tpl.size_z > 512)
			sb = getSBlockOrig(buildable.getBlocksCompleted());
		else
			sb = getSBlockRandom(buildable.getBlocksCompleted());
	...
	private void processWonderAbort() {
		CivMessage.sendTown(buildable.getTown(), CivColor.Rose
				+ CivSettings.localize.localizedString("var_buildAsync_wonderFarAway", buildable.getDisplayName()));

		// Refund the town half the cost of the wonder.
		double refund = (int) (buildable.getCost() / 2);
		buildable.getTown().depositDirect(refund);

		CivMessage.sendTown(buildable.getTown(), CivColor.Yellow + CivSettings.localize
				.localizedString("var_buildAsync_wonderRefund", refund, CivSettings.CURRENCY_NAME));
		abortWonder();
	}
